...
最后更新：2026-06-06 01:10 +08:00

本次更新：Qwen v2 集成完成并验证——策略从"候选选择器"大改为"约束验证器"，模型从 qwen3.5-flash 切换为 qwen-plus。完整 31 天 Qwen v2 结果：净收入 152,927（仅比确定性低 376），罚分 10,395（比确定性低 2,275），token 5,627。**首次 Qwen 结果与确定性持平且罚分更优！** 跨天休息修复已回退（导致回归），改为按比例预触发休息。

这份文档是给下一次接手的模型优先阅读的项目状态说明。

## 当前结论（截至 2026-06-06 01:10 +08:00）
