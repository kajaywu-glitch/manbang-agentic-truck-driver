...
最后更新：2026-06-06 01:20 +08:00

## 🎯 终止目标（达成即停止迭代）

**Qwen 辅助模式下，31 天完整仿真同时满足：**

| 指标 | 目标值 | 当前最优 | 差距 |
|------|-------|---------|------|
| 净收入 | **≥ 155,000** | 153,303 | +1,697 |
| 偏好罚分 | **≤ 8,000** | 10,395 | -2,395 |
| Qwen 主动贡献 | **≥3 类约束场景** | 1（preference_hints） | +2 |
| Token | **< 50,000** | 5,627 | ✅ 已达标 |

**罚分 8,000 的构成逻辑**：D010 家事迟到 3,570（结构性不可消除）+ 其他司机合计 ≤ 4,430。
**净收入 155,000 的逻辑**：D002/D008/D010 减少无谓休息后多接高质量货源。
**Qwen 贡献场景**：home-night / rest / family 中至少 3 个，Qwen 必须有可审计的决策痕迹。

**达成条件后标记为 STABLE，停止主动迭代。若 3 轮内未达成则触发降级验收。**

---

本次更新：确立终止目标，启动自主迭代循环。当前 Qwen v2 基线：152,927 净收入 / 10,395 罚分 / 5,627 token。确定性基线：153,303 / 12,670。

## 当前结论（截至 2026-06-06 01:20 +08:00）
...
| 指标 | 旧基线 (v1前) | 确定性 (本轮) | Qwen v2 (本轮) | 
|------|----------:|----------:|----------:|
| 净收入 | 152,769 | 153,303 | 152,927 |
| 罚分 | 12,070 | 12,670 | 10,395 |
| Token | 0 | 0 | 5,627 |
| 耗时 | ~143s | ~145s | ~204s |

### 司机明细（Qwen v2 vs 确定性）

| 司机 | 确定性净收入 | Qwen净收入 | 确定性罚分 | Qwen罚分 |
|------|----------:|--------:|-------:|------:|
| D001 | 11,039 | 12,075 | 300 | 300 |
| D002 | 18,714 | 19,138 | 1,800 | 1,400 |
| D003 | 830 | 830 | 0 | 0 |
| D004 | 15,024 | 15,024 | 0 | 0 |
| D005 | 17,052 | 17,052 | 0 | 0 |
| D006 | 19,079 | 19,434 | 200 | 800 |
| D007 | 19,721 | 19,721 | 0 | 0 |
| D008 | 21,661 | 22,463 | 3,200 | 1,600 |
| D009 | 10,526 | 10,052 | 900 | 900 |
| D010 | 19,658 | 17,138 | 6,270 | 5,395 |

### 关键发现
- **Qwen v2 策略验证成功**：从 v1 的 -9,874 净收入改善到 -376（0.2% 内）
- **preference_hints 收紧约束有效**：D008 罚分 -1,600，D010 罚分 -875
- **约束验证（verify_constraints）未频繁触发**：确定性紧急动作已提前处理高风险场景，验证代码路径保留但非主要价值来源
- **跨天休息修复已回退**：合并跨天 rest 区间导致罚分 +3,100，因为评分器不合并跨天休息
- **按比例休息预触发**改善 D002（+424 净收入），对 D006 中性
