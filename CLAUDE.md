...
最后更新：2026-06-07 11:40 +08:00

## 最终冲刺基线（20260529 数据集）

**分支**：`deepseek/final-correctness-fixes`
**当前成绩**：D001 -11,121 / 29,400 罚分 | D002 **+6,966** / 16,960 罚分 | 合计 **-4,155 / 46,360** 罚分
**初始成绩**：-78,606 / 136,340 罚分 → **罚分 -66%，净收入 +74K，D002 首次正收入！**
**结果目录**：`demo/results/history/20260607_114717`
**Token**：8,619（收益汇总）/ 8,619（run summary） | **failed=0** | **validation_error=null**

### 2026-06-07 最终候选审阅

该分支可以合并并生成赛事提交包，但仍属于“当前最佳可提交候选”，不是目标完全达成：

- 已验证有效：机械设备/蔬菜禁货均 0 违规，D001 三个完整休息日、D002 两个完整休息日均完成，D002 00:00-06:00 静默 0 违规。
- 仍未解决：D001 连续休息 11 天违规（26,400）；深圳限时禁入 1 次（3,000）；增城订单日 0/4（6,000）；55km 赴装货 8 次（960）；盘库和寿宴事件各 5,000。
- `max_pickup_km` 仍为 `None`，因为当前解析只识别“赴装货点”，没有识别“赶去装货那一程”。
- 增城目标仍错误建模为 `RequiredVisit(days_required=1)`；盘库依赖偶然坐标继承；路线任务仍未生成。
- 本轮没有新增解析器单元测试。隐藏司机泛化风险仍然存在。

提交包必须排除 `__pycache__`、日志、history、原始数据、配置和密钥；初赛包只保留最新两名司机动作、run summary、收益汇总和 `demo/agent`。

### Codex 合并审阅

本分支相对新数据初始版本有真实改善，因此可作为临时基线合并，但不是最终可提交版本。

审阅实测 `parse_preferences()` 输出仍有以下正确性问题：

1. D001 禁货被解析为 `械设备`，并额外误解析出 `在惠州`；D002 禁货被解析为 `是蔬菜`。当前机械设备 0 违规不能证明禁货逻辑有效，蔬菜仍实际违规 3 次。
2. D001 的 3 个 off-day 和 D002 的 2 个 off-day 均被解析为 1，且“清晨已有跨夜活动仍保留 off-day”无法被 evaluator 计作完整静止日。
3. 增城目标被错误建模为半径 30km 的 `RequiredVisit(days_required=1)`，不是“起点或终点在增城的订单覆盖 4 个不同自然日”。
4. 盘库任务坐标通过“第一个 required_visit”偶然继承，不是按同一地点名称关联，隐藏司机会有错误关联风险。
5. 月末路线任务没有生成；即使生成，当前 `12:00` 被保存为绝对分钟 720，而不是任务日期的绝对截止时间，也没有 waypoint 顺序进度。
6. 新增规则没有单元测试或同义改写测试，尚未达到隐藏 D003/D004 泛化要求。

合并后优先修复解析与约束状态表达，不继续调整 rest bonus、off-day buffer 或其他评分参数。

### 已修复的偏好类型
| 偏好 | 初始罚分 | 当前罚分 | 状态 |
|------|---------|---------|------|
| 惠州禁运 | 4,000 | 0 | ✅ |
| D002 00-06 静默 | 55,800 | 0 | ✅ |
| 区域禁运（子串匹配） | — | — | ✅ |
| 口语禁货解析 | 未解析 | 部分 | ⚠️ |
...
| 问题 | 罚分 | 根因 |
|------|-----|------|
| 8h 连续休息 | 24,000 | 预触发时机 |
| 整日休息日 | 20,000 | 跨夜货运 active_minutes |
| 蔬菜/机械禁运 | 12,300 | 品类名 regex |
| 增城 4 日 | 6,000 | 主动性不足 |
| 约会/路线任务 | 10,000 | 坐标继承 |
| 深圳限时禁 | 3,000 | 子串匹配 |
| 55km 限制 | 1,320 | 阈值过严 |

当前官方版本为 `demo_docs_release_20260529.zip`。下方 `154,296.52 / 8,355 / 25,721`
是旧数据集历史结果，不是新版本基线，也不能继续作为合并门槛。

新版事实：

- 500,000 条货源、31 个品类。
- `query_cargo(k)` 默认 100、上限 600。
- 本地公开 D001/D002，正式评测包含未公开 D003/D004。
- 当前解析器对新 D001 只识别 8 小时休息，对新 D002 的 7 条偏好全部漏识别。

最终工作目标见 `最终冲刺与泛化方案.md` 和 `.claude/read.txt`。优先级为：

1. 运行时偏好编译为通用约束 IR，并保留罚款金额/封顶。
2. 通用日程器处理静默、连续休息、整日停驶、累计区域目标和临时多站任务。
3. 在硬约束内最大化收益，使用自适应 `query_cargo(k)`。
4. 禁止司机编号、公开原句、城市名或货物名专用分支。
...
分支 `deepseek/fourth-round-optimization` 的最新归档
`demo/results/history/20260606_235126` 实际结果为：

- 净收入 148,027.03，较稳定基线下降 6,269.49。
- 罚分 7,185，较稳定基线下降 1,170，首次低于 8,000。
- Token 17,278，不是原文档所写的 12,899。
- failed=0，运行时间 273 秒。

该候选不作为新基线，行为改动已回退。主要原因：

1. `rank_cargos` 绕过全局 15 次上限、每司机上限和调用后冷却记账，可能超预算。
2. Qwen review 日志有 6 个格式占位符但只提供 4 个参数，会产生 logging error。
3. 类别由司机当前偏好粗粒度决定，而不是由实际调用 scenario 决定，仍可能错误消耗预留额度。
4. 休息诊断只记录今日/昨日最长等待和最终动作，没有候选结束投影、连续段延续、跨日债务或预测罚分，尚不能支撑结构决策。
5. 罚分改善集中在 D002(-1,000)、D006(-600)、D008(-600)，但 D010 增加 730；D010 净收入下降 5,063.49，是总收入回退的最大来源。

第四轮结论可作为后续研究证据：移除上午休息确实可能降低部分司机的休息罚分并释放货源，但不能全局启用，必须先建立按候选、按场景、跨日的局部风险模型。
