...
| 指标 | 终极目标 | **低罚分候选** | 状态 |
|------|---------|------------:|------|
| 净收入 | ≥160,000 | **153,541** | ❌ 差 6,459 |
| 罚分 | ≤8,000 | **8,355** | ⚠️ 差 355 |
| Token | <50,000 | 64,567 | ❌ 差 14,567 |
| Qwen 场景 | ≥3 | **3**（rank/suggest/verify） | ✅ |
| failed=0 | 必须 | **0** | ✅ |

**低罚分候选 commit**：`04d88ae`（上午主动休息 + 休息兼容性奖励）
**关键突破**：上午主动休息（≤4h 司机）将罚分从 12,955 降至 **8,355**（-35.5%），距 8,000 目标仅差 355。
净收入从 156,973 降至 153,541（-2.2%），为罚分改善付出的代价。
该结果与旧净收入冠军互不支配，应作为 Pareto 候选而不是全面冠军。

---

本次更新：架构重构成功——前置休息调度 + 货源休息兼容性奖励，罚分首次进入 8,000 区间。
...
- 稳定分支：`main`
- **低罚分结果实际配置**：总复审 20、每司机最多 5 次、rank 最多 5 次，模型 `qwen3.5-flash`
- **当前代码默认配置**：`AGENT_QWEN_MAX_REVIEWS=15`, `AGENT_QWEN_MAX_REVIEWS_PER_DRIVER=5`, `AGENT_QWEN_MAX_RANKS=5`
- **Codex 审阅修正**：修复 D009 home-night 校验读取不存在字段导致的降级异常；该修正等待 CC 下一轮结果确认。
...
| 改动 | 文件 | 影响 |
|------|------|------|
| `net_per_hour` 0.25→0.5 | planner.py | +2,837 确定性净收入 |
