...
本次更新：官方 2026-05-29 新数据集已接入。旧版成绩全部降级为历史参考；本地仅公开 D001/D002，隐藏评测还有 D003/D004。后续不再进行旧样本逐司机调参，直接按 `最终冲刺与泛化方案.md` 和 `.claude/read.txt` 完成通用偏好编译、约束日程和新数据集最终优化。
...
5. `最终冲刺与泛化方案.md`：当前官方数据版本的最终冲刺设计。
...
## 进度显示要求（更新于 2026-05-29 19:42 +08:00）

完整仿真或 Qwen 短测必须让用户能看到评测进度。这里的“进度”不是普通开发日志，而是用户能直接判断仿真跑到哪里、有没有卡住、当前结果是否正常。
...
- 每条 heartbeat 至少包含：当前司机、当前司机仿真日期、当前 step、已完成司机数、累计 token、Qwen 是否正在调用或刚刚 fallback、运行耗时。能拿到结果时，显示已完成司机的阶段摘要；如果收益必须等 `calc_monthly_income.py`，至少显示动作数、是否报错、最后仿真时间。

- 每条 heartbeat 建议格式：

```text
[SIM_PROGRESS] driver=D001 day=2026-03-04 step=12/200 completed_drivers=0/10 action=take_order qwen=rank_cargos total_tokens=5745 elapsed_ms=48720 status=running
```
